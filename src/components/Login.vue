<template>
  <!-- Aplicamos la fuente a todo el contenedor principal -->
  <div class="min-h-screen bg-[#050505] bg-[radial-gradient(circle_at_center,_#0a192f_0%,_#050505_100%)] flex flex-col items-center justify-center p-6 font-app antialiased">
    ...
    <div class="w-full max-w-md bg-[#0d1117] rounded-[2.5rem] p-10 shadow-2xl border border-white/5">
      ...
      <!-- Logo centrado sin cuadro -->
      <div class="flex justify-center mb-10">
        <img src="../assets/logo.png" alt="Logo" class="w-28 h-auto object-contain filter brightness-125" />
      </div>

      <div class="text-center mb-10">
        <h1 class="text-3xl font-bold text-white tracking-tight animate-fade-in">Bienvenido</h1>
      ...
      <!-- Botón ACCEDER con Hover a Blanco y Brillo -->
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (res.data.status === 'success') {
      ...
      // Guardamos el token, el rol y el nombre para usarlos en toda la app
      ...
      if (res.data.role === 'admin') {
        ...
        router.push('/parametros'); // Redirige a Parametro.vue
      }
    }
  } catch (error) {
    if (error.response) {
